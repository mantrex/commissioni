...
const commNum = ref('00000000')
const checking = ref(false)
const valid = ref(null)   // null=non verificato, true=libero, false=esiste
const error = ref('')
...
const checkCommNum = async (val) => {
  ...
  try {
    const result = await $fetch(`/api/orders/check-commnum?commNum=${encodeURIComponent(val)}`)
    if (result.exists) {
      valid.value = false

...
export async function migrateAgents(mysqlConn, Agent, stats) {
  ...
  try {
    ...
    log.success(
      `Agents: ${stats.agents.imported}/${stats.agents.total} imported`,
    );
    return agentsMap;
  } catch (err) {
...
export async function migrateClients(mysqlConn, Client, stats) {
  ...
  try {
    ...
    log.success(
      `Clients: ${stats.clients.imported}/${stats.clients.total} imported`,
    );
    return clientsMap;
  } catch (err) {
  ...
  try {
    ...
    for (const row of rows) {
      try {
        ...
        log.error(
          `Failed to import product ${row.IDMerceVend}: ${err.message}`,
        );
        stats.products.failed++;
      }
    }
    ...
    log.success(
      `Products: ${stats.products.imported}/${stats.products.total} imported`,
    );
    return productsMap;
  } catch (err) {
...
export async function migrateOrders(
  ...
  try {
    ...
    for (const row of rows) {
      try {
        ...
        if (row.Sped1data || row.Corr1) {
          shipments.push({
            date: row.Sped1data || null,
            courier: row.Corr1 || "",
          });
        }
        if (row.Sped2data || row.Corr2) {
          shipments.push({
            date: row.Sped2data || null,
            courier: row.Corr2 || "",
          });
        }
        if (row.Sped3data || row.Corr3) {
          shipments.push({
            date: row.Sped3data || null,
            courier: row.Corr3 || "",
          });
        }
    ...
    log.success(
      `Orders: ${stats.orders.imported}/${stats.orders.total} imported`,
    );
    return ordersMap;
  } catch (err) {
...
export async function migrateInvoices(
  ...
  try {
    ...
    log.success(
      `Invoices: ${stats.invoices.imported}/${stats.invoices.total} imported`,
    );
  } catch (err) {
